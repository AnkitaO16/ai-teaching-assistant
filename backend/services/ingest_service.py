...
# ✅ Set global embedding model (used everywhere in the app)
Settings.embed_model = HuggingFaceEmbedding(model_name="BAAI/bge-small-en-v1.5")


def ingest_text_file(file_path: str, class_name: str, subject: str, topic: str) -> dict:
    """
    Ingest a teacher's text file into a Chroma collection.
    Collection name format is standardized via make_collection_name().
    """
    try:
        # 1. Load file into LlamaIndex docs
        documents = SimpleDirectoryReader(input_files=[file_path]).load_data()
        ...
        # 4. Wrap collection in LlamaIndex vector store
        vector_store = ChromaVectorStore(chroma_collection=collection)
        storage_context = StorageContext.from_defaults(vector_store=vector_store)
